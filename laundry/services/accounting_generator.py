...
class AccountingEntryGenerator:
    ...
    def _determine_entries(self):
        ...
        cash_account = self._get_or_create_account('Cash', 'asset', 'D')
        accounts_receivable = self._get_or_create_account('Accounts Receivable', 'asset', 'D')
        accounts_payable = self._get_or_create_account('Accounts Payable', 'liability', 'C')
    ...
    def _get_or_create_account(self, name, account_type, normal_balance, code=None):
        """Get or create an account by name and type"""
        try:
            if code:
